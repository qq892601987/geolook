...
项目目录：技能根目录下的 `work/<slug>/`，每个客户/产品一个目录。
...
界面为暗色四段主线（AtlasGEO）：**现状**（总览 / 引擎表现 / 竞品对比 / 问题库）→ **诊断**（差距诊断 / 阵地地图 / 品牌事实库）→ **提升**（行动计划 / 内容工作台）→ **成效**（效果验收 / 报告与交付），外加设置与三步接入引导。统一口径：GEO 健康分（五项加权的**诊断性就绪度指标**，未测项权重归一）、提及率、引用份额、阵地、任务、内容。总览标题由数据自动生成；算不出的显示「未测」，不编数。健康分和审计分不是被引用概率或效果保证。
...
- 归因默认写"观察相关"，除非有基线窗口 + 对照 Prompt + 竞品对照；新版测量只有问题面板、实体目录、终端、联网状态、模型和 route 都一致时才显示跨期变化，且变化仍不是因果结论。
- 历史采样会保留为 `legacy` 回放；首个完整新版采样是新基线。`0%`、`未测`、`样本不完整`、`不可比较`四种状态必须分开解释。
...
> 每周一早上在技能根目录运行 `python3 scripts/geo.py serve --slug <项目>`，
