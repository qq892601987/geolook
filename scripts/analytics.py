...
def _latest_metric(pdir: Path):
    files = sorted((pdir / "metrics").glob("*.json")) if (pdir / "metrics").exists() else []
    return G.read_json(files[-1], None) if files else None


def _rows_for_metrics(pdir: Path, metrics: dict | None):
    if not metrics:
        return []
    run_id = metrics.get("run_id")
    if run_id:
        path = pdir / "samples" / f"{metrics.get('date')}-{run_id}.jsonl"
        return _rows(path)
    files = _sample_files(pdir)
    return _rows(files[-1]) if files else []
...
def build(slug: str) -> dict:
    ...
    bp = G.read_json(pdir / "blueprint.json", None)
    fc = G.read_json(pdir / "factcheck.json", []) or []
    metrics = _latest_metric(pdir)
    rows_latest = _rows_for_metrics(pdir, metrics)

    return {
        "latest_date": metrics.get("date") if metrics else None,
        "measurement": {"legacy": bool(metrics and metrics.get("schema_version") != 2),
                        "quality": (metrics or {}).get("quality", {}),
                        "comparable": bool((metrics or {}).get("comparison_identity"))},
