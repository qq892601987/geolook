...
def _measurement_is_compatible(task: dict, metrics: dict) -> bool:
    baseline = task.get("baseline_measurement")
    if not baseline:
        return True
    if metrics.get("schema_version") != 2 or (metrics.get("quality") or {}).get("status") != "complete":
        return False
    return (baseline.get("question_panel_fingerprint") ==
            (metrics.get("question_panel") or {}).get("fingerprint"))
...
def check(task: dict, audit: dict, metrics: dict) -> tuple[bool | None, str, dict | None]:
    ...
    try:
        ...
        if expr.startswith("metrics.mention_rate_gte:"):
            if not _measurement_is_compatible(task, metrics or {}):
                return None, "本期测量与工单基线不可比较或样本不完整", None
            _, mk, tgt = expr.split(":")
