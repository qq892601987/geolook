...
def prev_metrics(pdir: Path, metrics: dict | None):
    files = sorted((pdir / "metrics").glob("*.json"))
    current_run_id = (metrics or {}).get("run_id")
    previous = None
    for f in reversed(files):
        candidate = G.read_json(f, None)
        if candidate and (not current_run_id or candidate.get("run_id") != current_run_id):
            previous = candidate
            break
    if not previous:
        return None
    if not metrics or metrics.get("schema_version") != 2:
        return previous
    import sample
    return previous if sample.metrics_are_comparable(metrics, previous) else None


def measurement_note(metrics: dict | None, previous: dict | None) -> str:
    if not metrics:
        return "本期没有采样数据。"
    if metrics.get("schema_version") != 2:
        return "历史采样快照：缺少新版面板/协议指纹，保留回放但不用于严格趋势比较。"
    quality = metrics.get("quality") or {}
    if quality.get("status") != "complete":
        return ("本期样本不完整：成功 %(successful_samples)s/%(expected_samples)s，"
                "不输出可比较趋势或指标验收结论。" % quality)
    if previous is None:
        return "本期为新的可比测量基线；后续需保持同一问题面板、实体目录和采样协议才显示趋势。"
    return "本期与上一期使用相同问题面板、实体目录与采样协议；变化仅为观察性趋势，不代表因果归因。"
...
def build_markdown(cfg, audit, metrics, prev_m, prev_a, todos) -> str:
    ...
    if readiness and readiness.get("status") != "ready":
        A("## 品牌底座复核状态")
        A("")
        A("**品牌事实、竞品或问题库尚待人工复核；本报告与内容资产只能作为草案使用。**")
        A("请在工作台核对事实卡、问题库和竞品清单后，再用于对外发布或客户签收。")
        A("")

    run_status = G.load_run_status(cfg["slug"]) if cfg.get("slug") else {}
    if run_status and run_status.get("status") != "completed":
        A("## 本期证据状态与限制")
        A("")
        A("**本期为基础交付，不能将未测指标解释为品牌表现。**")
        for stage in run_status.get("stages", []):
            if stage.get("status") in ("partial", "skipped", "failed"):
                A(f"- {stage.get('name')}：{stage.get('reason') or stage.get('status')}")
        A("")
    ...
    if not metrics or not metrics.get("platforms"):
        ...
        A("")
        A("> " + measurement_note(metrics, prev_m))
        A("")
